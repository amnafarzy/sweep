...
The packaged app appears in `dist/` as a branded `.app` and a styled `.dmg`
(app and `/Applications` drop-target side by side). The app icon and DMG
backdrop live in `build/` and are generated by a dependency-free Node script —
see [`build/README.md`](build/README.md) to regenerate them or rebuild
`icon.icns` via `iconutil` on a Mac.

On first launch, macOS Gatekeeper may warn it's from an unidentified developer — right-click the app → Open to bypass (it's unsigned because it's your own local build).
