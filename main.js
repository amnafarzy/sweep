...
const FORBIDDEN = new Set([
  '/', HOME, '/System', '/Library', '/usr', '/bin', '/sbin', '/etc',
  '/var', '/private', '/Applications', '/Users',
...
function isInsideHome(p) {
  ...
  return resolved !== HOME && !rel.startsWith('..') && !path.isAbsolute(rel);
}

function assertSafeToRemove(p) {
  if (!p || typeof p !== 'string') throw new Error('Invalid path');
  const resolved = path.resolve(p);
  if (resolved.length < 5) throw new Error('Refusing to operate on root-level path');
  if (FORBIDDEN.has(resolved)) throw new Error(`Refusing to touch protected path: ${resolved}`);
  const isAppBundle = /^\/Applications\/[^/]+\.app$/.test(resolved);
  if (!isAppBundle && !isInsideHome(resolved)) throw new Error(`Refusing to operate outside home: ${resolved}`);
  return resolved;
}
...
async function getMemory() {
  try {
    ...
    // Everything physical not otherwise accounted for is cached/file-backed memory.
...
// Precise leftover finder: match on bundle identifier or an exact/prefixed app
// name — never a loose "*name*" substring (which would sweep up unrelated files).
